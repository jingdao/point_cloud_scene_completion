...
class PCDVisualizer(PointCloudVisualizer):
    ...
    def __init__(self, skip=None, near=0, far=float('inf'), rotate=0):
        if skip == None:
...
class DepthMapVisualizaer(PointCloudVisualizer):
    ...
    def __init__(self, camera, skip=None, near=0, far=float('inf'), rotate=0):
        if skip == None:
            PointCloudVisualizer.__init__(self)
        else:
            PointCloudVisualizer.__init__(self, skip)
...
if __name__ == '__main__':
    ...
    vis_pcd.show_pc_from_pcd('./0.pcd')
    # vis_pcd.show_pc_from_pcd('./wall_with_hole.pcd')