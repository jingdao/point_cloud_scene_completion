...
Examples = collections.namedtuple("Examples", "paths, inputs, targets, interp, count, steps_per_epoch")
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        if a.lab_colorization:
            ...
            b_images = preprocess(raw_input[:,width//3:width//3*2,:])
            c_images = preprocess(raw_input[:,width//3*2:,:])

    if a.which_direction == "AtoB":
        inputs, targets, interp = [a_images, b_images, c_images]
    ...
    with tf.name_scope("interp_images"):
        interp_images = transform(interp)

    paths_batch, inputs_batch, targets_batch, interp_batch = tf.train.batch([paths, input_images, target_images, interp_images], batch_size=a.batch_size)
...
def create_model(inputs, targets, interp):
    inputs_interp = tf.concat([inputs, interp], axis=3)
    ...
    with tf.variable_scope("generator"):
        ...
        outputs, predict_mask_logits = create_generator(inputs_interp, out_channels)
...
def main():
    ...
    model = create_model(examples.inputs, examples.targets, examples.interp)
...
#            print("wrote index at", index_path)
            ...
            for step in range(max_steps):
                ...
                if should(a.progress_freq):
                    ...
                    print("discrim_loss %.3f gen_loss_GAN %.3f gen_loss_L1 %.3f mask_loss %.3f prc %.3f rcl %.3f"% 
                        (results["discrim_loss"], results["gen_loss_GAN"], results["gen_loss_L1"], results["mask_loss"], results['prc'], results['rcl']))
